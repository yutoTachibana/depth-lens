...
| Finding | Why it matters |
|---|---|
| [**v2.0 — Inference-time compute scaling laws across 3 paradigms**](docs/findings/v2.0-scaling-law.md) | Token-CoT API · Self-hosted vLLM · Looped (OpenMythos 1M/10M/100M) all on **one FLOPs axis** across 5 tasks. **Research finding**: a 1M-param looped model beats gpt-5-mini by 24,000-410,000× on FLOPs on 4/5 bounded-depth synthetic tasks — *not a production recommendation*. The ratio is real but applies only when (1) the task has a bounded input distribution, (2) you can train a custom model per task, and (3) call volume amortizes the engineering overhead. For typical chatbots/RAG/agents, the v1.0-v1.2 findings still apply: use the cheap API tier. See the [finding doc's "When this applies" section](docs/findings/v2.0-scaling-law.md#when-this-finding-applies--and-when-it-doesnt) before quoting the ratio. |
